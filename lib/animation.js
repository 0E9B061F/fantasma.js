...
class Playback {
    constructor(animation, conf) {
        ...
        this.beforeSkip = this.parent ? null : this.conf.beforeSkip
        this.afterSkip = this.parent ? null : this.conf.afterSkip
    ...
    skip(t) {
        if (this.beforeSkip) this.beforeSkip(this)
        t = Math.min(1, Math.max(0, t))
        this.elapsed = this.animation.totalTime * t
        this.rt = t
        this.t = this.animation.easing(t)
        this.lastf = Date.now()
        this.set(this.t)
        this.pause()
        if (this.afterSkip) this.afterSkip(this)
    }
    frame() {
